...
class SaslMechanismPlain(SaslMechanism):
    def __init__(self, **config):
        ...
        assert "sasl_plain_username" in config, (
            "sasl_plain_username required for PLAIN sasl"
        )
        assert "sasl_plain_password" in config, (
            "sasl_plain_password required for PLAIN sasl"
        )
    ...
    def auth_bytes(self):
        ...
        # HACK: kafka-python should not be setting the first parameter here to the username, but rather to
        # the 'zid'. See (as they actually note) RFC-4616.
        return bytes("\0".join(["", self.username, self.password]).encode("utf-8"))
